...
<p>Crime is a pervasive issue that affects communities across the United States, and understanding the factors that contribute to crime rates is crucial for developing effective strategies to reduce crime and improve public safety. This report aims to explore the relationship between crime rates and socioeconomic factors, specifically focusing on the unemployment rate, in community areas of Chicago. By analyzing crime data and demographic data from 2022, we seek to determine if there is a significant correlation between the unemployment rate and the crime rate, with a particular emphasis on violent crime. The motivation behind this analysis is to provide insights that can inform policy decisions and resource allocation aimed at reducing crime and promoting safer communities. The thesis statement of this report is as follows: Based on the analysis of crime and demographic data from Chicago in 2022, there is a significant positive correlation between the unemployment rate and the crime rate, particularly violent crime, in community areas of the city.</p>

<h2>Question of Interest</h2>

<p>The primary question of interest in this analysis is: Is there a significant correlation between the unemployment rate and the crime rate, particularly violent crime, in Chicago community areas?</p>

<h2>Thesis Statement</h2>

<p>Based on the analysis of crime and demographic data from Chicago in 2022, there is a significant positive correlation between the unemployment rate and the crime rate, particularly violent crime, in community areas of the city.</p>

<h1>Background</h1>

<p>The analysis in this report utilizes two main datasets: crime data and demographic data for Chicago in 2022. The crime data is obtained from the "Crimes - 2001 to Present" dataset, which is publicly available on the Chicago Data Portal <a href="https://catalog.data.gov/dataset/crimes-2001-to-present">(<https://catalog.data.gov/dataset/crimes-2001-to-present>)</a>. This dataset contains information about reported crimes in Chicago, including the date, primary type of crime, and the community area where the crime occurred. The data is collected and maintained by the Chicago Police Department.</p>

<p>The demographic data is sourced from the "Community Data Snapshots Raw Data 2014-2022" dataset, specifically the "CCA_2022" layer, which is available on the Chicago Metropolitan Agency for Planning (CMAP) Data Hub <a href="https://datahub.cmap.illinois.gov/datasets/CMAPGIS::community-data-snapshots-raw-data-2014-2022/explore?layer=21">(<https://datahub.cmap.illinois.gov/datasets/CMAPGIS>::community-data-snapshots-raw-data-2014-2022/explore?layer=21)</a>. This dataset provides information about the population, unemployment rate, and other socioeconomic indicators for each community area in Chicago.</p>

<p>The key variables used in the analysis are:</p>

<ul>

<li>Crime_Percent: The percentage of crimes in each community area relative to the total population.</li>

<li>Unemployment_rate: The unemployment rate in each community area, calculated as the number of unemployed individuals divided by the total population.</li>
...
<p>Before diving into the analysis, let's take a quick look at the data.</p>

<h3>Crime Data 2022 (First 5 rows)</h3>
...
demographics_2022_original <- read_csv("~/Documents/Career/School/UW-Madison/24Spring/Stat240/final project/data/2022.csv", show_col_types = FALSE)
...
<h3>Demographic Data 2022 (First 5 rows)</h3>
...
<h3>Crime Count by Community Area</h3>
...
<h3>Crime Counts by Community Area</h3>

<p>This bar plot shows the total crime count for each community area in Chicago. It provides an overview of which areas have the highest and lowest crime counts.</p>
...
<p>This bar plot displays the total population for each community area in Chicago. It helps to understand the population distribution across different areas.</p>
...
<h3>Distribution of Crime</h3>

<p>This density plot illustrates the distribution of total crime counts across community areas. It shows the shape of the distribution and identifies any potential outliers or unusual patterns.</p>
...
<h3>Distribution of Population</h3>

<p>This density plot represents the distribution of population across community areas. It provides insights into the population density and identifies areas with high or low population concentrations.</p>
...
<h3>Distribution of Unemployment Rate</h3>

<p>This density plot displays the distribution of unemployment rates across community areas. It provides an overview of the unemployment situation in different parts of Chicago.</p>
...
<h2>Part 3: Visualization - Geo Maps</h2>

<p>To visualize the spatial distribution of crime and unemployment across Chicago, we create geo maps using the crime and demographic data.</p>
...
chicago_shapefile <- st_read("~/Documents/Career/School/UW-Madison/24Spring/Stat240/final project/Boundaries - Community Areas (current)/geo_export_9e592439-888c-4b65-9398-039b0cfb6192.shp")
...
<h3>Map 1: General Crime Rate</h3>

<ul>

<li>Description: This map shows the distribution of the general crime rate across community areas in Chicago.</li>
...
<h3>Map 2: Unemployment Rate</h3>

<ul>

<li>Description: This map shows the distribution of the unemployment rate across community areas in Chicago.</li>

<li>Interpretation: The map highlights the spatial variability in unemployment rates, with certain community areas experiencing higher levels of unemployment compared to others.</li>
...
<h3>Map 3: Violent Crime Rate</h3>

<ul>

<li>Description: This map shows the distribution of the violent crime rate across community areas in Chicago.</li>

<li>Interpretation: The map identifies hotspots of violent crime, indicating the community areas that are disproportionately affected by violent offenses.</li>

</ul>

<h1>Discussion:</h1>

<p>The analysis presented in this report sheds light on the relationship between crime rates and unemployment in Chicago community areas. While the hypothesis test in Part 1 did not find a significant correlation between the general crime rate and unemployment rate, the focused analysis in Part 2 revealed a strong positive correlation between the unemployment rate and the violent crime rate.</p>
...
<li>Correlation coefficient: 0.115</li>

<li>p-value: 0.321</li>

<li>95% confidence interval: (-0.112, 0.3301)</li>

</ul>

</li>

<li>Interpretation: The correlation coefficient of 0.115 suggests a weak positive correlation between the unemployment rate and the general crime rate. However, the p-value of 0.321 is greater than the significance level of 0.05, indicating that there is not enough evidence to reject the null hypothesis. The 95% confidence interval includes zero, further supporting the lack of a significant correlation.</li>

</ul>

<p>Conducting Pearson's product-moment correlation test between the unemployment rate and the violent crime rate yields the following results:</p>

<ul>

<li>Correlation coefficient: 0.780</li>

<li>p-value: \< 2.2e-16</li>

<li>95% confidence interval: (0.673, 0.854)</li>

</ul>

<p>The correlation coefficient of 0.780 indicates a strong positive linear relationship between the unemployment rate and the violent crime rate. The p-value is extremely small (\< 2.2e-16), providing strong evidence to reject the null hypothesis of no correlation. The 95% confidence interval does not include zero, further supporting the presence of a significant positive correlation.</p>

<p>These findings have important implications for policymakers and law enforcement agencies. They suggest that addressing unemployment and economic disparities in communities may have a positive impact on reducing violent crime. Targeted interventions and programs aimed at improving employment opportunities and providing support to individuals in high-unemployment areas could be effective strategies for promoting public safety.</p>

<p>However, it is crucial to acknowledge the limitations of this analysis. The study relies on reported crimes, which may not capture the full extent of criminal activity. Additionally, the analysis focuses on a single year of data and does not consider other socioeconomic factors that may influence crime rates.</p>

<p>Future research could expand on this work by incorporating additional variables, such as education levels, income inequality, and access to social services. Conducting a longitudinal analysis over multiple years could provide insights into the temporal dynamics of the relationship between unemployment and crime. Furthermore, qualitative studies involving community stakeholders and law enforcement personnel could offer valuable context and inform policy recommendations.</p>
...
<h3>Geo Maps Interpretation</h3>

<h4>Map 1: General Crime Rate</h4>

<p>Description: This map displays the spatial distribution of the general crime rate across different community areas in Chicago. The darker shades of blue represent higher crime rates, while lighter shades indicate lower crime rates.</p>

<p>Correlation with the topic: Although the analysis did not find a significant correlation between the general crime rate and unemployment rate, this map provides valuable spatial context by revealing areas with concentrated crime hotspots. These hotspots may coincide with regions experiencing higher unemployment rates and other socioeconomic challenges, as suggested by the focused analysis on violent crime and unemployment.</p>

<h4>Map 2: Unemployment Rate</h4>

<p>Description: This map visualizes the spatial variability in unemployment rates across community areas in Chicago. The darker shades of green represent higher levels of unemployment, while lighter shades indicate lower unemployment rates.</p>

<p>Correlation with the topic: This map directly relates to the central focus of the analysis, which is the relationship between unemployment and crime rates. The spatial distribution of unemployment rates shown in this map can be compared with the crime rate maps to identify potential overlaps or patterns. Areas with higher unemployment rates may correspond to regions with elevated crime rates, particularly for violent crimes, as supported by the strong positive correlation found in Part 2 of the analysis.</p>

<h4>Map 3: Violent Crime Rate</h4>

<p>Description: This map illustrates the distribution of the violent crime rate across different community areas in Chicago. The darker shades of red signify higher rates of violent crime, while lighter shades represent lower rates.</p>
...
<p>The findings emphasize the importance of addressing unemployment and economic disparities as part of a comprehensive approach to crime prevention and public safety. Policymakers and law enforcement agencies should consider targeted interventions and programs to improve employment opportunities and support individuals in high-unemployment areas.</p>

<p>However, further research is needed to fully understand the complex dynamics between crime and socioeconomic factors. For example, we found a strong correlation between of total crimes and also certain type of crimes with the time period they occur.</p>
...
t <- b / 0.06157
pt(t, df = 75, lower.tail = TRUE)
race_vars <- c("P_WHITE", "P_HISP", "P_BLACK", "P_ASIAN", "P_OTHER")

```

By expanding the analysis to include additional variables, conducting longitudinal studies, and incorporating qualitative insights, we can gain a more comprehensive understanding of the factors that contribute to crime and develop evidence-based strategies to promote safer communities. By expanding the analysis to include additional variables, conducting longitudinal studies, and incorporating qualitative insights, we can gain a more comprehensive understanding of the factors that contribute to crime and develop evidence-based strategies to promote safer communities.
